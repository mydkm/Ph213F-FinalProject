...
def sr_carpet_generation(
    ...
    Returns:
      ...
      rects_side (list of float): side length of each square in rects_phys
    """
    if n < 1:
    ...
    def paint_square(cx, cy, side):
        ...
        if i1 > i0 and j1 > j0:
            ...
            rects_side.append(side)  # record side length
...
def run_sim(cfg: WaveConfig):
    ...
    # white borders for every square, with depth-dependent line width
    for (rect, side) in zip(rects_phys, rects_side):
        x0p, y0p, wp, hp = rect
        lw = CARPET_BORDER_LW * (side / cfg.base_len)
        lw = max(lw, 0.5)  # keep tiny squares visible
