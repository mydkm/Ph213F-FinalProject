...
# Keep this paired with figsize below so the saved video is exactly 1920x1080.
VIDEO_DPI = 100

CARPET_FACE_RGBA    = (0.12, 0.08, 0.20, 0.95)
CARPET_BORDER_COLOR = (1.0, 1.0, 1.0, 1.0)

# Base border width. Smaller squares scale down from this,
...
def sr_carpet_generation(
    ...
    rects_phys : list[tuple]
        Each tuple is (x0, y0, width, height), in physical coordinates.

    rects_side : list[float]
        Side length corresponding to each rectangle.
    """
    if n < 1:
    ...
    def paint_square(cx, cy, side):
        ...
        i0 = int(round((cx - 0.5 * side) / dx))
        j0 = int(round((cy - 0.5 * side) / dy))
        ...
        if i1 > i0 and j1 > j0:
            ...
            rects_side.append(side)

    # Level 1: centered base square
    ...
    return open_mask, rects_phys, rects_side
...
class WaveConfig:
    ...
    # For faster previews, use --N 720 or --N 540.
    N: int = 540
    ...
    save_mp4: bool = True
    mp4_fname: str = "wave_centered_carpet_cpu_1080p.mp4"
...
def run_sim(cfg: WaveConfig):
    ...
    open_mask, rects_phys, rects_side = sr_carpet_generation(
        N,
        cfg.n,
        cfg.base_len,
        Lx,
    ...
    # nearest interpolation and resample=False keep the carpet mask crisp.
    ...
    # White borders:
    # Clamp minimum linewidth to 1.0 for better 1080p visibility.
    for rect, side in zip(rects_phys, rects_side):
        x0p, y0p, wp, hp = rect
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        description="CPU-only 2D wave simulation with growing carpet obstacle."
    )
